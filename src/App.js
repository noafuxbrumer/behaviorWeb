...
function App() {
  ...
  const watchShowTZ = watch("showTZ", false);
  const watchAllFields = watch(); // when pass nothing as argument, you are watching everything
  const watchFields = watch(["showMark","showTZ", "number"]); // you can also target specific fields by their names
  ...
  return (
    <>
     ...
     <label>TZ</label>
        ...
        />
        {errors.address && (
          <p>{"The Address Field is Required and must be > 49 characters"}</p>
        )}
